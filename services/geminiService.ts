...
import { RestorationOptions, DamageType, Intensity, FacePreservation, PhotoType } from "../types";
...
export const buildPrompt = (options: RestorationOptions): string => {
  ...
  if (options.damageTypes.length > 0) {
    ...
    const repairs = [];
    if (options.damageTypes.includes(DamageType.SCRATCHES)) repairs.push("Remove all visible scratches and fine lines");
    if (options.damageTypes.includes(DamageType.TEARS)) repairs.push("Mend visible tears and rips seamlessly");
    if (options.damageTypes.includes(DamageType.FADING)) repairs.push("Restore faded areas to their original tonal range");
    if (options.damageTypes.includes(DamageType.STAINS)) repairs.push("Clean localized stains and discoloration");
    if (options.damageTypes.includes(DamageType.CRACKS)) repairs.push("Fill in cracks and emulsion fractures");
    prompt += repairs.join(". ") + ".\n\n";
  }
  ...
  switch (options.facePreservation) {
    case FacePreservation.STRICT:
      prompt += "Preserve facial features, expressions, and proportions with absolute fidelity. Maintain exact eye color, nose shape, mouth structure, and facial contours.\n\n";
      break;
    case FacePreservation.MODERATE:
      prompt += "Enhance facial clarity while maintaining strong resemblance to the original subject.\n\n";
      break;
    case FacePreservation.FLEXIBLE:
      prompt += "Enhance facial details for clarity, prioritizing a natural look over pixel-perfect adherence if the original is extremely blurry.\n\n";
      break;
  }

  // Colorization
  if (options.photoType === PhotoType.B_AND_W && options.colorize) {
    prompt += "Colorize this black-and-white image with natural, period-appropriate colors. Ensure skin tones are realistic and clothing colors match the historical era.\n\n";
  }

  // Grain
  if (options.preserveGrain) {
    prompt += "Preserve the original film grain and texture characteristics to maintain authenticity.\n\n";
  } else {
    prompt += "Reduce noise and grain for a cleaner, modern digital look, but avoid a 'waxy' appearance.\n\n";
  }

  // Local Repairs
  if (options.localRepairRegions.length > 0) {
    prompt += "Local repairs needed in the following regions (coordinates are approximate percentages):\n";
...
export class GeminiService {
  ...
  public async startRestoration(file: File, options: RestorationOptions): Promise<{ imageUrl: string, prompt: string }> {
